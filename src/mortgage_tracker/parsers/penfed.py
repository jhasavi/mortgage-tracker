"""Parser for PenFed Credit Union mortgage rates."""
import logging
...
logger = logging.getLogger("mortgage_tracker.parsers.penfed")


class PenFedParser(BaseParser):
    """Parser for PenFed Credit Union mortgage rates."""
    ...
    def parse(self, *, text: Optional[str] = None, js: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        try:
            # PenFed uses tables - try multiple column configurations
            offers = extract_offers_from_html_table(
                ...
                rate_col=1,
                apr_col=2,
                points_col=3,  # May have points column
                min_columns=3
            )
