"""Parser for State Employees' Credit Union (NCSECU)."""
import logging
...
logger = logging.getLogger("mortgage_tracker.parsers.ncsecu")


class NCSECUParser(BaseParser):
    """Parser for NCSECU mortgage rates."""
    ...
    def parse(self, *, text: Optional[str] = None, js: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        try:
            # NCSECU typically has simple rate tables
            offers = extract_offers_from_html_table(
                html=text,
                lender_name="State Employees' Credit Union (NCSECU)",
                product_col=0,
                rate_col=1,
                apr_col=2,
                points_col=None,
